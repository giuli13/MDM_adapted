...
from data_loaders.truebones.utils.motion_process import cont6d_to_matrix_torch
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.lambda_ric_pos = lambda_ric_pos
    ...
    def attention_scores_loss(self, attn_scores, adj_mask, lengths, temp_mask, timesteps):
        # assuming temp_mask.shape == bs, 1, 1, seqlen
        # assuming adj_mask.shape == bs, max_joints, max_joints
        # assuming attn_scores.shape == bs, seqlen, max_joints, max_joints
        intervene_from_step = 50
        consider_loss = torch.zeros_like(timesteps)
        consider_loss[ timesteps >=intervene_from_step] = 1.0
        temp_mask = temp_mask.permute(0, 3, 1, 2)
        reg_const = 0.01
        binary_adj_mask = adj_mask.unsqueeze(1) 
        # attn_loss = self.l2_loss(adj_mask, attn_scores) * binary_adj_mask # .long for new mask only 
        attn_loss = attn_scores * (-binary_adj_mask)
        attn_loss = sum_flat(attn_loss * temp_mask.float()) * consider_loss
        non_zero_elements = torch.count_nonzero(binary_adj_mask, (1,2,3)) * lengths
        loss = attn_loss / non_zero_elements
        return loss * reg_const
    
    def parents_feature_loss(self, x, parents, temp_mask, spat_mask, lengths, n_joints):
        bs = x.size(0)
        # assuming x.shape == bs, J, Jdim, seqlen
        # assuming parents.shape == bs, J
        parent_feats_strat_ind = 16
        orig_parents_feats = x[:, parents[:, 1:], :parent_feats_strat_ind-1, :][torch.arange(bs), torch.arange(bs)]
        concat_parents_feats = x[:, 1:, parent_feats_strat_ind:, :]
        loss = self.l2_loss(orig_parents_feats, concat_parents_feats)
        temp_masked_loss = loss * temp_mask.float()
        spat_temp_masked_loss = (temp_masked_loss.transpose(1,3) * spat_mask.float()[:, :, :, 1:]).transpose(1,3)
        loss = sum_flat(spat_temp_masked_loss)  # gives \sigma_euclidean over unmasked elements
        non_zero_elements = lengths * (n_joints-1) * 15
        mse_loss_val = loss / non_zero_elements
        return mse_loss_val

    def temporal_spatial_masked_l2(self, a, b, temp_mask, spat_mask, lengths, n_joints, epoch):
        # assuming a.shape == b.shape == bs, J, Jdim, seqlen
        # assuming temp_mask.shape == bs, 1, 1, seqlen
        ...
        # mask root joint irrelevant feats 
        spat_temp_masked_loss[:, 0, 4:, :] = 0.0 #root joints has 4 feats only
        loss = sum_flat(spat_temp_masked_loss)  # gives \sigma_euclidean over unmasked elements
        non_zero_elements = lengths * n_joints * a.size(2) - (lengths * 9)
        mse_loss_val = loss / non_zero_elements
        return mse_loss_val
    ...
    def get_root_rot_pos(self, data):
        ...
        root_rot_mat = self.quat_to_mat(r_rot_quat)
    ...
    def recover_from_bvh_ric(self, data):
        r_rot, r_pos = self.get_root_rot_pos(data.permute(0, 3, 1, 2)[..., 0:1, :4])
    ...
    def recover_ric_positions(self, data):
        #r_rot, r_pos = get_root_rot_pos_test(data.permute(0, 3, 1, 2)[..., 0:1, 3:9])
        relative_positions = data.permute(0, 3, 1, 2)[..., 1:, :3]
        '''Add Y-axis rotation to local joints'''
        '''Concate root and joints'''
        r_pos = torch.zeros(relative_positions.shape[0], relative_positions.shape[1], 1, relative_positions.shape[3], device=data.device)
        r_pos[..., 1] = data.permute(0, 3, 1, 2)[..., 0:1, 3]
        # r_pos.requires_grad=data.r_pos
    ...
    def geodesic_loss(self, a, b, temp_mask, spat_mask, lengths, n_joints):
        ...
        # assuming temp_mask.shape == bs, 1, 1, seqlen
        # assuming spat_mask.shape == bs, 1, 1, max_joints
        rots_target = cont6d_to_matrix_torch(a.permute(0, 3, 1, 2)[..., 1:, 3:9])
        rots_pred = cont6d_to_matrix_torch(b.permute(0, 3, 1, 2)[..., 1:, 3:9])
        loss = geodesic_distance(rots_pred, rots_target).permute(0, 2, 3, 1)
        temp_masked_loss = loss * temp_mask.float()
        spat_temp_masked_loss = (temp_masked_loss * spat_mask[..., 1:].float().transpose(1,3))
        # mask root joint irrelevant feats 
        loss = sum_flat(spat_temp_masked_loss)  # gives \sigma_euclidean over unmasked elements
    ...
    def geodesic_loss_with_root(self, a, b, temp_mask, spat_mask, lengths, n_joints, target_root_rot, pred_root_rot):
        # assuming a.shape == b.shape == bs, J, Jdim, seqlen
        # assuming temp_mask.shape == bs, 1, 1, seqlen
        # assuming spat_mask.shape == bs, 1, 1, max_joints
        rots_target = cont6d_to_matrix_torch(a.permute(0, 3, 1, 2)[..., 1:, 3:9])
        rots_pred = cont6d_to_matrix_torch(b.permute(0, 3, 1, 2)[..., 1:, 3:9])
        loss = geodesic_distance(torch.cat([pred_root_rot, rots_pred], dim=-3), torch.cat([target_root_rot, rots_target], dim=-3)).permute(0, 2, 3, 1)
        temp_masked_loss = loss * temp_mask.float()
        spat_temp_masked_loss = (temp_masked_loss * spat_mask.float().transpose(1,3))
        # mask root joint irrelevant feats 
        loss = sum_flat(spat_temp_masked_loss)  # gives \sigma_euclidean over unmasked elements
    ...
    def foot_sliding_loss(self, a, b, temp_mask, spat_mask, lengths, n_joints, relative=True):
        # assuming a.shape == b.shape == bs, J, Jdim, seqlen
        # assuming temp_mask.shape == bs, 1, 1, seqlen
        # assuming spat_mask.shape == bs, 1, 1, max_joints
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None, epoch=-1):
        """
        ...
        enc = model.model
        if 'lengths_mask' in model_kwargs['y']:
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if lengths is not None: # truebones
                # calc all loss terms 
                terms["l_simple"] = self.temporal_spatial_masked_l2(target, model_output, mask, joints_mask, lengths, actual_joints, epoch)
                ...
                # # calc all loss terms 
                if self.lambda_ric_pos > 0.:
                    target_ric_positions = self.recover_ric_positions(target)
                    pred_ric_positions = self.recover_ric_positions(model_output)
                    terms["ric_pos_mse"] = self.temporal_spatial_masked_l2(target_ric_positions.permute(0, 2, 3, 1), pred_ric_positions.permute(0, 2, 3, 1), mask, joints_mask, lengths, actual_joints, epoch)
                    terms["loss"] =  terms["loss"] + self.lambda_ric_pos * terms["ric_pos_mse"]
                if self.lambda_geo > 0.:    
                    target_r_rot, target_r_pos = self.get_root_rot_pos(target.permute(0, 3, 1, 2)[..., 0:1, :4])
                    pred_r_rot, pred_r_pos = self.get_root_rot_pos(model_output.permute(0, 3, 1, 2)[..., 0:1, :4])
                    terms["geodesic_loss"] = self.geodesic_loss_with_root( target, model_output, mask, joints_mask, lengths, actual_joints, target_r_rot, pred_r_rot)
                    terms["loss"] = terms["loss"] + self.lambda_geo * terms["geodesic_loss"]
                if self.lambda_fs > 0.:
                    terms["foot_sliding_loss"] = self.foot_sliding_loss(target, model_output, mask, joints_mask, lengths, actual_joints, relative=True)
