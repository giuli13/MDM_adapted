...
def get_opt(opt_path, device, hml=False):
    opt = Namespace()
    opt_dict = vars(opt)
    ...
    if opt.dataset_name == 't2m':
        ...
        opt.dim_pose = 251
        opt.max_motion_length = 196
    elif opt.dataset_name == 't2m2p':
        if hml:
            opt.data_root = './dataset/HumanML3D'
            opt.motion_dir = pjoin(opt.data_root, 'new_joint_vecs')
            opt.cond_file = './dataset/truebones/zoo/processed/hml_cond.npy' 
            opt.text_dir = pjoin(opt.data_root, 'texts')
            opt.max_motion_length = 40
            opt.max_joints = 22
            opt.feature_len = 13
        else:
            opt.data_root = './dataset/truebones/zoo/processed_bvh_rots'
