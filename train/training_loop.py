...
from sample.generate_gmdm import main as generate_gmdm
from sample.generate import main as generate_mdm
from sample.generate_baseline import main as generate_baseline
import copy
...
class TrainLoop:
    def __init__(self, args, train_platform, model, diffusion, data):
        ...
        if args.dataset in ['kit', 'humanml', 'truebones', 'humanml_mat'] and args.eval_during_training:
    ...
    def generate_during_training(self):
        ...
        gen_args.num_samples = 1 if self.args.hml else self.args.gen_num_samples
        ...
        if self.dataset == 'truebones':
            ...
            all_objects = self.data.dataset.t2m_dataset.cond_dict.keys() 
            random.seed(self.step)
            gen_args.object_type = random.sample(all_objects, gen_args.num_samples)
            random.seed(self.args.seed)
            all_sample_save_path = generate_baseline(gen_args, self.data.dataset.t2m_dataset.cond_dict)
        else:
            ...
            all_sample_save_path = generate_mdm(gen_args)
