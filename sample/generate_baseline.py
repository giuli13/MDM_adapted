...
def main(args = None, cond_dict = None):
    ...
    _, model_kwargs = create_condition(object_types, cond_dict, n_frames, args.temporal_window, tpos_not_normalized = args.tpos_not_normalized)
    ...
    for rep_i in range(args.num_repetitions):
        ...
        for i, motion in enumerate(sample):
            ...
            np.save(pjoin(out_path, npy_name), global_positions)
...
def create_condition(object_types, cond_dict, n_frames, temporal_window, tpos_not_normalized=False):
    batches = list()
    for object_type in object_types:
        ...
        batch.append(torch.zeros(n_joints, 1, dtype=torch.int32))
        batch.append(0)
        batch.append(mean)
        batch.append(std)
