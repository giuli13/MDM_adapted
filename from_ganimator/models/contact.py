...
def constrain_from_contact(contact, glb, fid='TBD', L=5):
    ...
    for i, fidx in enumerate(fid):  # fidx: index of the foot joint
        ...
        while s < T:
            ...
            # find the average position of the contact period
            t = s  # frame index where the contact starts
            avg = glb[t, fidx].clone()
            while t + 1 < T and fixed[t + 1] == 1:
                t += 1
                avg += glb[t, fidx].clone()
            avg /= (t - s + 1)  # s,t hold contact period start,end respectively

            # override original locations with the average position
